...
int speechSynthesizerFile(const char* appkey) {
	...
	string tmpTex = "今日天气真不错，我想去操作踢足球.";
	pa.text = GBKToUTF8(tmpTex);  // windows下默认编码是GBK，必须转换成UTF-8编码
...
int speechSynthesizerMultFile(const char* appkey) {
	...
	vector<HANDLE> threadHandle(AUDIO_TEXT_NUMS);
	vector<unsigned> pthreadId(AUDIO_TEXT_NUMS);
