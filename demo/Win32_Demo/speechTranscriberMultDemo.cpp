...
struct ParamStruct {
	...
	WORD startedConsumed;
	WORD completedConsumed;
	WORD closeConsumed;

	WORD failedConsumed;
	WORD requestConsumed;

	WORD startTotalValue;
	WORD startAveValue;
	WORD startMaxValue;
	WORD startMinValue;
...
public:
	...
	SYSTEMTIME startTv;
	SYSTEMTIME startedTv;
	SYSTEMTIME completedTv;
	SYSTEMTIME closedTv;
	SYSTEMTIME failedTv;
...
void onTranscriptionStarted(NlsEvent* cbEvent, void* cbParam) {
	...
	GetLocalTime(&(tmpParam->startedTv));

	//    pthread_mutex_lock(&(tmpParam->tParam->mtx));
	tmpParam->tParam->startedConsumed++;

	WORD timeValue1 = tmpParam->startedTv.wSecond - tmpParam->startTv.wSecond;
	WORD timeValue2 = tmpParam->startedTv.wMilliseconds - tmpParam->startTv.wMilliseconds;
	WORD timeValue = 0;
	if (timeValue1 > 0) {
		timeValue = (((timeValue1 * 1000) + timeValue2));
	}
	else {
		timeValue = (timeValue2);
	}

	cout << "S0 Time: " << tmpParam->startedTv.wSecond << " " << tmpParam->startTv.wSecond<< endl;
	cout << "S1 Time: " << tmpParam->startedTv.wMilliseconds << " " << tmpParam->startTv.wMilliseconds << endl;
...
void onTranscriptionCompleted(NlsEvent* cbEvent, void* cbParam) {
	...
	GetLocalTime(&(tmpParam->completedTv));

	//    pthread_mutex_lock(&(tmpParam->tParam->mtx));
	tmpParam->tParam->completedConsumed++;

	WORD timeValue1 = tmpParam->completedTv.wSecond - tmpParam->startTv.wSecond;
	WORD timeValue2 = tmpParam->completedTv.wMilliseconds - tmpParam->startTv.wMilliseconds;
	WORD timeValue = 0;
	if (timeValue1 > 0) {
		timeValue = (((timeValue1 * 1000) + timeValue2));
	}
	...
	cout << "C0 Time: " << tmpParam->completedTv.wSecond << " " << tmpParam->startTv.wSecond << endl;
	cout << "C1 Time: " << tmpParam->completedTv.wMilliseconds << " " << tmpParam->startTv.wMilliseconds << endl;
...
void onSentenceSemantics(NlsEvent* cbEvent, void* cbParam) {
	...
	cout << "onSentenceSemantics: All response:" << cbEvent->getAllResponse() << endl; // 获取服务端返回的全部信息
}
...
void onChannelClosed(NlsEvent* cbEvent, void* cbParam) {
	...
	WaitForSingleObject(tmpParam->mtx, INFINITE);
	tmpParam->isContinue = true;
	ReleaseMutex(tmpParam->mtx);
...
unsigned __stdcall pthreadFunction(void* arg) {
	...
	do {
		...
		*/
		...
		request->setToken("84b2c63c10fb4e61b318c29a49c7bd03");
	...
	*/
    if (ret == 0) {
		...
		WaitForSingleObject(cbParam->mtx, INFINITE);
		while (!cbParam->isContinue) {
			ReleaseMutex(cbParam->mtx);
			Sleep(100);
		}
		ReleaseMutex(cbParam->mtx);
    }
...
//    }
	...
	sAveTime /= AUDIO_FILE_NUMS;
	eAveTime /= AUDIO_FILE_NUMS;
...
int main(int arc, char* argv[]) {
	...
	//启动工作线程
	NlsClient::getInstance()->startWorkThread(0);

    // 识别多个音频数据
    speechTranscriberMultFile("123");
