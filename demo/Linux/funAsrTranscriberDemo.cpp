...
std::string g_token = "";
std::string g_url = "";
std::string g_model = "fun-asr-realtime";
...
std::string g_log_file = "log-funasr-transcriber";
int g_log_count = 20;
int g_threads = 1;
...
void onTaskFailed(AlibabaNls::NlsEvent* cbEvent, void* cbParam) {
  ...
  FILE* failed_stream = fopen("funasrTranscriptionTaskFailed.log", "a+");
  if (failed_stream) {
  ...
  if (result) {
    ...
    switch (cbEvent->getMsgType()) {
      case AlibabaNls::NlsEvent::TaskFailed:
        ...
        failed_stream = fopen("funasrTranscriptionTaskFailed.log", "a+");
        if (failed_stream) {
          std::string ts = timestampStr(NULL, NULL);
          char outbuf[1024] = {0};
          snprintf(outbuf, sizeof(outbuf),
...
/**
 * @brief 短链接模式下工作线程
 *     以 createDashFunAsrTranscriberRequest    <----|
 *                   |                               |
 *           request->start()                        |
 *                   |                               |
 *           request->sendAudio()                    |
 ...
 *           收到onChannelClosed回调                  |
 *                   |                               |
 *    releaseDashFunAsrTranscriberRequest(request)  ----|
 *        进行循环。
 */
void* pthreadFunction(void* arg) {
  ...
  do {
    ...
    /*
     * 1. 创建实时音频流识别DashFunAsrTranscriberRequest对象
     */
    AlibabaNls::DashFunAsrTranscriberRequest* request =
        AlibabaNls::NlsClient::getInstance()
            ->createDashFunAsrTranscriberRequest("cpp", longConnection);
    if (request == NULL) {
      std::cout << "createDashFunAsrTranscriberRequest failed." << std::endl;
      delete cbParam;
      cbParam = NULL;
      return NULL;
    }
    ...
    if (ret < 0) {
      ...
      AlibabaNls::NlsClient::getInstance()->releaseDashFunAsrTranscriberRequest(
          request);
      break;
    } else {
      if (g_sync_timeout == 0) {
...
/**
 * @brief 长链接模式下工作线程
 *           createDashFunAsrTranscriberRequest
 *                          |
 *        然后以    request->start() <----------|
 *                          |                   |
 *                  request->sendAudio()        |
...
void* pthreadLongConnectionFunction(void* arg) {
   ...
   */
  AlibabaNls::DashFunAsrTranscriberRequest* request =
      AlibabaNls::NlsClient::getInstance()->createDashFunAsrTranscriberRequest(
          "cpp", longConnection);
  if (request == NULL) {
    std::cout << "createDashFunAsrTranscriberRequest failed." << std::endl;
    delete cbParam;
  ...
  if (parse_argv(argc, argv)) {
    std::cout
        ...
        << "  --breakTimeEachRound <Set the interval time for each round, and "
           ...
           "0.>\n"
        << "eg:\n"
        << "  ./funAsrTranscriberDemo --apikey xxxxxx\n"
        << "  ./funAsrTranscriberDemo --apikey xxxxxx --threads 4 --time 3600\n"
        << "  ./funAsrTranscriberDemo --apikey xxxxxx --threads 4 --time 3600 "
           "--log 4 --type pcm\n"
        << "  ./funAsrTranscriberDemo --apikey xxxxxx --threads 1 --loop 1 "
