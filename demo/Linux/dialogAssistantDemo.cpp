...
void OnDialogResultGenerated(AlibabaNls::NlsEvent* cbEvent, void* cbParam) {
  ...
  std::cout
      ...
      << cbEvent->getTaskId()  // 当前任务的task id, 方便定位问题, 建议输出.
      << std::endl;
  std::cout << "OnDialogResultGenerated: All response:"
...
void OnRecognitionChannelClosed(AlibabaNls::NlsEvent* cbEvent, void* cbParam) {
  ParamCallBack* tmpParam = static_cast<ParamCallBack*>(cbParam);
  std::cout << "OnRecognitionChannelClosed userId: " << tmpParam->userId << ", "
            << tmpParam->userInfo << std::endl;  // 仅表示自定义参数示例
 ...
 * @return
 */
void OnWakeWordVerificationCompleted(AlibabaNls::NlsEvent* cbEvent,
                                     void* cbParam) {
  ParamCallBack* tmpParam = static_cast<ParamCallBack*>(cbParam);

  std::cout << "CbParam: " << tmpParam->userId << ", " << tmpParam->userInfo
            << std::endl;  // 仅表示自定义参数示例
...
void* TextDialogPthreadFunc(void* arg) {
  ...
  request->setContextParam(
      "{\"network\":{\"ip\":\"100.101.102.103\"}, "
      "\"custom\":{\"dingding_user_id\":\"xxx\"}}");
...
void* AudioDialogPthreadFunc(void* arg) {
  ...
  while (global_run) {
    ...
    AlibabaNls::NlsClient::getInstance()->releaseDialogAssistantRequest(
        request);
...
int dialogAssistantMultFile(const char* appkey, int threads) {
  ...
  for (int j = 0; j < threads; j++) {
    pthread_create(&pthreadId[j], NULL, &AudioDialogPthreadFunc,
                   (void*)&(pa[j]));  //音频进文本出.
    // pthread_create(&pthreadId[j], NULL, &TextDialogPthreadFunc, (void
    // *)&(pa[j])); //文本进文本出.
  }
...
int main(int argc, char* argv[]) {
  if (parse_argv(argc, argv)) {
    std::cout << "params is not valid.\n"
              ...
              << "  ./daDemo --appkey xxxxxx --akId xxxxxx --akSecret xxxxxx "
                 "--threads 4 --time 3600\n"
              << std::endl;
    return -1;
  }
