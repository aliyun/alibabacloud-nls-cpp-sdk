...
std::string g_model = "cosyvoice-v2";
std::string g_voice = "longxiaochun_v2";
std::string g_log_file = "log-cosyvoice-Synthesizer";
int g_log_count = 20;
int g_threads = 1;
...
static bool wordTimestamp = false;
...
void OnSynthesisTaskFailed(AlibabaNls::NlsEvent* cbEvent, void* cbParam) {
  ...
  FILE* failed_stream = fopen("cosyvoiceSynthesisTaskFailed.log", "a+");
  if (failed_stream) {
    std::string ts = timestampStr(NULL, NULL);
    char outbuf[1024] = {0};
...
void* autoCloseFunc(void* arg) {
  ...
  while (loop && global_run) {
    ...
    if (global_sys) {
      PROFILE_INFO cur_sys_info;
      get_profile_info("cosyvoiceSynthesizerDemo", &cur_sys_info);
...
/**
 * @brief 短链接模式下工作线程
 *        以 createDashCosyVoiceSynthesizerRequest   <----|
 *                   |                              |
 *           request->start()                       |
 *                   |                              |
 *           request->sendText()                    |
 ...
 *                   |                              |
 *      releaseDashCosyVoiceSynthesizerRequest(request) --|
 *        进行循环。
 */
void* pthreadFunc(void* arg) {
  ...
  while (global_run) {
    ...
    /*
     * 1. 创建流式文本语音合成DashCosyVoiceSynthesizerRequest对象.
     *
     * 流式文本语音合成文档详见:
    ...
    AlibabaNls::DashCosyVoiceSynthesizerRequest* request =
        AlibabaNls::NlsClient::getInstance()
            ->createDashCosyVoiceSynthesizerRequest();
    if (request == NULL) {
      std::cout << "createDashCosyVoiceSynthesizerRequest failed." << std::endl;
      break;
    }
    ...
    request->setSpeechRate(1.0);
    request->setPitchRate(1.0);
    request->setWordTimestampEnabled(wordTimestamp);
...
void* pthreadLongConnectionFunc(void* arg) {
  ...
  AlibabaNls::DashCosyVoiceSynthesizerRequest* request =
      AlibabaNls::NlsClient::getInstance()
          ->createDashCosyVoiceSynthesizerRequest("cpp", longConnection);
  if (request == NULL) {
    std::cout << "createDashCosyVoiceSynthesizerRequest failed." << std::endl;
    return NULL;
  }
  ...
  while (global_run) {
    ...
    if (!text_str.empty()) {
      ...
      if (ret < 0) {
        ...
        // 此request可能已经发生TaskFailed并CLOSED，尽量不要调用request的接口以防崩溃。
        const char* request_info = request->dumpAllInfo();
        AlibabaNls::NlsClient::getInstance()
            ->releaseDashCosyVoiceSynthesizerRequest(
                request);  // start()失败，释放request对象
  ...
  AlibabaNls::NlsClient::getInstance()->releaseDashCosyVoiceSynthesizerRequest(
      request);
  request = NULL;
...
int flowingSynthesizerMultFile(int threads) {
...
int parse_argv(int argc, char* argv[]) {
  ...
  while (index < argc) {
    ...
    } else if (!strcmp(argv[index], "--wordTimestamp")) {
      index++;
      if (invalid_argv(index, argc)) return 1;
      if (atoi(argv[index])) {
        wordTimestamp = true;
...
int main(int argc, char* argv[]) {
  if (parse_argv(argc, argv)) {
    std::cout
        << "params is not valid.\n"
        << "DashScope CosyVoice stream input synthesizer usage:\n"
        << "  --apikey <apikey>\n"
        ...
        << "  --wordTimestamp <enable word timestamp, default 0>\n"
        << "  --model <set model, default cosyvoice-v2>\n"
        << "  --voice <set voice, default longxiaochun_v2>\n"
        ...
        << "  --breakTimeEachRound <Set the interval time for each round, and "
           ...
           "0.>\n"
        << "eg:\n"
        << "  ./cosyvoiceSynthesizerDemo --appkey xxxxxx --token xxxxxx\n"
        << "  ./cosyvoiceSynthesizerDemo --appkey xxxxxx --akId xxxxxx "
           "--akSecret xxxxxx "
           "--threads 4 --time 3600\n"
        << "  ./cosyvoiceSynthesizerDemo --appkey xxxxxx --akId xxxxxx "
           "--akSecret xxxxxx "
  ...
  for (cur_profile_scan = -1; cur_profile_scan < profile_scan;
    ...
    // 合成多个文本
    ret = flowingSynthesizerMultFile(g_threads);
    if (ret) {
