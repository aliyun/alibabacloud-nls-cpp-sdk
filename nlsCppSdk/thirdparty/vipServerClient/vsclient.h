...
参数：VIPSrv_Config：cache_dir：本地容灾文件根路径；
                                          如：cache_dir = getenv("HOME") 当前用户目录下
	           VIPSrv_Config：jmenv_dom：地址服务器域名；一般为jmenv.tbsite.net
...
参数：VIPSrv_Result* ：接口调用的返回结果的指针；记录接口调用是否与成功，以及IP地址信息
******/
...
extern VIPSrv_Result* async_vipsrv_srv_clusters_iplist(const char*, const char*);
...
/******
功能：返回指定userId所属单元的所有IP列表，此IP列表是 dom 域名对应IP列表的一个子集返，
                 包括"可用"与"不可用"的IP，标记区分，若不能查找到，则返回错误
参数：domain：vipserver中的域名，对应一组ip；
	  		 userId：要查询的用户ID；
返回值：VIPSrv_Result* 记录查询是否成功，以及查询到的IP列表的具体信息
...
*******/
extern VIPSrv_Result* vipsrv_srv_unit_hosts(const char* domain, uint64_t userId);
extern VIPSrv_Result* async_vipsrv_srv_unit_hosts(const char* domain, uint64_t userId);
/*******
功能:封装router接口暴露给外部
