...
public:
    ...
    *
    * 异步请求成功回调此接口
    * @param[in] iphosts 获取到的IPHost信息，如果是获取一个那么数组长度为1，数组长度一定>=1
    */
...
| 环境变量名                            | 意义                          | 默认值                            | 备注                                            |
+--------------------------------------+-------------------------------+----------------------------------+-------------------------------------------------+
| VIPCLIENT_SET_C_MAX_BLOCK_TIME_MS    | C客户端所有同步接口的超时时间    | 5000                             | 单位毫秒                                        |
+--------------------------------------+-------------------------------+----------------------------------+-------------------------------------------------+
| VIPCLIENT_SET_ALOG_CONFIGURE_FILE    | 日志配置文件                   | ""                               | 程序无需配置文件                                 |
+--------------------------------------+-------------------------------+----------------------------------+-------------------------------------------------+
| VIPCLIENT_SET_LOG_LEVEL              | 日志级别                       | 4                                | 5-DEBUG,4-INFO,3-WARN,2-ERROR,1-FATAL,0-DISABLE |
+--------------------------------------+-------------------------------+----------------------------------+-------------------------------------------------+
| VIPCLIENT_SET_LOG_PATH               | 日志路径                       | "."                              | 确保写权限，会创建vipclient-logs                 |
+--------------------------------------+-------------------------------+----------------------------------+-------------------------------------------------+
| VIPCLIENT_SET_PROCESS_SHARE          | 共享内存开关                   | 0                                | 1-打开，0-关闭                                   |
+--------------------------------------+-------------------------------+----------------------------------+-------------------------------------------------+
| VIPCLIENT_SET_PROCESS_SHARE_PATH     | 共享内存的控制路径              | "/tmp/.vipclient-process-share/" | 保证目录有写权限                                 |
+--------------------------------------+-------------------------------+----------------------------------+-------------------------------------------------+
| VIPCLIENT_SET_MAX_CONCURRENT_REQUEST | 客户端最大并发请求数            | 1000                             | 最小为10，最大为3000                             |
+--------------------------------------+-------------------------------+----------------------------------+-------------------------------------------------+
| VIPCLIENT_SET_BAD_QUERY_SPAN_SECONDS | 同一个错误域名的请求间隔时间    | 10                               | 单位秒，最小值5秒，最大值5*60秒                    |
+--------------------------------------+-------------------------------+----------------------------------+-------------------------------------------------+
| VIPCLIENT_SET_CACHE_PATH             | 缓存路径                      | "."                              | 确保写权限，会创建vipclient-cache                 |
+--------------------------------------+-------------------------------+----------------------------------+-------------------------------------------------+
| VIPCLIENT_SET_LOCAL_CACHE_FIRST      | 设置是否本地缓存优先获取        | 0                                | 0： 内存-->服务器---->本地缓存                    |
|                                      |                               |                                  | 1： 内存-->本地缓存-->服务器                      |
+--------------------------------------+-------------------------------+----------------------------------+-------------------------------------------------+
| VIPCLIENT_ENABLE_AUTH                | 开启或关闭鉴权和多租户功能      | 0                                | 0： 关闭                                         |
|                                      |                               |                                  | 1： 开启                                        |
+--------------------------------------+-------------------------------+----------------------------------+-------------------------------------------------+
...
public:
    ...
    *               false 失败，可通过Errno获取错误码
    */
    static bool QueryAllIp(const char* domain, IPHostArray* iphosts, long timeout_ms);

    static bool QueryAllValidIp(const char* domain, IPHostArray* iphosts, long timeout_ms);

    static bool QueryUnitAllIp(const char* domain, const char* unit, IPHostArray* iphosts, long timeout_ms);

    static bool QueryClusterIp(const char* domain,
                               const char* cluster,
                               IPHost* iphost,
                               long timeout_ms);

    static bool QueryClusterAllIp(const char* domain,
                                  const char* cluster,
                                  IPHostArray* iphosts,
                                  long timeout_ms);
    ...
    */
    ...
    static bool TryQueryUnitAllIp(const char* domain, const char* unit, IPHostArray* iphosts);

    static bool TryQueryClusterIp(const char* domain,
                                  const char* cluster,
                                  IPHost* iphost);

    static bool TryQueryClusterAllIp(const char* domain,
                                     const char* cluster,
                                     IPHostArray* iphosts);
    ...
    *                 false  失败，可通过Errno获取错误码
    */
    static bool AsyncQueryIp(const char* domain, QueryHandler* handler, long timeout_ms);


    static bool AsyncQueryAllIp(const char* domain, QueryHandler* handler, long timeout_ms);


    static bool AsyncQueryClusterIp(const char* domain,
                                    const char* cluster,
                                    QueryHandler* handler,
                                    long timeout_ms);

    static bool AsyncQueryClusterAllIp(const char* domain,
                                       const char* cluster,
                                       QueryHandler* handler,
                                       long timeout_ms);
    ...
    * @param[in] iphosts    需要添加的IPHost信息数组，注意ip、port、weight为必填值，其余值不关心
    * @param[in] token      域名秘钥，可从登陆OPS来查看
    * @param[in] timeout_ms 超时时间
    * @return bool    true  成功
    *                 false 失败，可通过Errno获取错误码
    */
    static bool AddIpToDomain(const char* domain, const IPHostArray& iphosts, const char* token, long timeout_ms);
    static bool AddIpToDomain(const char* domain, const char* iphosts, const char* token, long timeout_ms);
    ...
    * @param[in] domain
    * @param[in] iphosts    需要删除的IPHost信息数组，注意ip、port、weight为必填值，其余值不关心
    * @param[in] token    域名秘钥，可从登陆OPS来查看
    * @param[in] timeout_ms    超时时间
    * @return bool    true 成功
    *                 false 失败，可通过Errno获取错误码
    */
    static bool RemoveIpFromDomain(const char* domain, const IPHostArray& iphosts, const char* token, long timeout_ms);
    static bool RemoveIpFromDomain(const char* domain, const char* iphosts, const char* token, long timeout_ms);
    ...
    *                 false 失败，可通过Errno获取错误码
    */
    static bool GetUnitByUserId(unsigned long long id, char* out_buff, unsigned int buff_len, long timeout_ms);
    ...
    *                 false 失败，可通过Errno获取错误码
    */
    static bool GetUnitByIp(const char* ip, char* out_buff, unsigned int buff_len, long timeout_ms);
