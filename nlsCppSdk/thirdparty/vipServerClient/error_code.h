...
enum ErrorCode {
    ...
    TRY_AGAIN_500MS     = -20004,    //请求已发出，需过段时间获取结果，TryXXX系列函数会返回此值
    TOO_MANY_REQUEST    = -20005,    //过度请求，同时请求的域名数过多导致
    ACCESS_CACHE_FAILED = -20006,    //写缓存失败，初始化时如果缓存目录无权限返回此值

    INVALID_VIPCLUSTER_DOMAIN      = -30001,    //不合法的vipserver域名，无法解析的域名
    NO_VIPSERVER_AVALIABAL         = -30002,    //无vipserver机器可用
    CONNECT_TO_VIPCLUSTER_FAIL     = -30003,    //连接vipserver集群失败，或者异步接口未回调前调用关闭接口
