...
public:
    ConnectNode() {};
    ConnectNode(INlsRequest* request, HandleBaseOneParamWithReturnVoid<NlsEvent>* handler);
    virtual ~ConnectNode();
    ...
    void handlerEvent(const char* error, int errorCode, NlsEvent::EventType eventType);
	void handlerTaskFailedEvent(std::string failedInfo);

	WorkThread* _eventThread;
    evutil_socket_t _socketFd;
    OpusEncoder* _opuEncoder;
    urlAddress _url;
...
//		char tmp_msg[1024] = {0};
//		snprintf(tmp_msg, 1024 - 1, "{\"TaskFailed\":\"%s\"}", _nodeErrMsg.c_str());
//		_nodeErrMsg = tmp_msg;
...
#if defined(__APPLE__)
    int _ios_dns(char *ip, int ipLen, int* aiFamily);
#endif
...
#if defined(__ANDROID__)
	int GetInetAddressByHostname(char *ipTmp, int* aiFamily);
#endif
};
